...
class HAM_loss(nn.Module):
    ...
    def forward(self, input, target):   ## Input: [b,1,512,512]    Target: [b,1,512,512]

        b, t, h, w = input.size()

        ## target surrounding = 2
        template = torch.ones(1, 1, 2*self.ProtectedArea+1, 2*self.ProtectedArea+1).to(input.device)    ## [1,1,5,5]
        target_prot = F.conv2d(target.float(), template, stride=1, padding=self.ProtectedArea)          ## [b,1,512,512]
        ...
        with torch.no_grad():
            ...
            WgtData_New = F.conv2d(WgtData_New, template, stride=1, padding=self.ProtectedArea)
